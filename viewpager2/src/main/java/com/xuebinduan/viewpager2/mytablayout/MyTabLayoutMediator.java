...
/**
 * A mediator to link a MyTabLayout with a ViewPager2. The mediator will synchronize the ViewPager2's
 * position with the selected tab when a tab is selected, and the MyTabLayout's scroll position when
 * the user drags the ViewPager2. MyTabLayoutMediator will listen to ViewPager2's OnPageChangeCallback
 * to adjust tab when ViewPager2 moves. MyTabLayoutMediator listens to MyTabLayout's
 ...
 * adapter and then call {@link #attach()} on it. Instantiating a MyTabLayoutMediator will only create
 * the mediator object, {@link #attach()} will link the MyTabLayout and the ViewPager2 together. When
 ...
 * {@link #attach()} call. Changing the ViewPager2 or MyTabLayout will require a new instantiation of
 * MyTabLayoutMediator.
 */
public final class MyTabLayoutMediator {
    @NonNull private final MyTabLayout tabLayout;
    ...
    @Nullable private MyTabLayout.OnTabSelectedListener onTabSelectedListener;
    ...
    public void attach() {
        if (attached) {
            throw new IllegalStateException("MyTabLayoutMediator is already attached");
        }
        adapter = viewPager.getAdapter();
        if (adapter == null) {
            throw new IllegalStateException(
                    "MyTabLayoutMediator attached before ViewPager2 has an " + "adapter");
        }
        attached = true;
    ...
    private static class TabLayoutOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
        ...
        TabLayoutOnPageChangeCallback(MyTabLayout tabLayout) {
     ...
     */
    private static class ViewPagerOnTabSelectedListener implements MyTabLayout.OnTabSelectedListener {
